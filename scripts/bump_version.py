#!/usr/bin/env python3
"""
Version bumping script for Nexus-Nancy.
...
Features:
- Updates version in pyproject.toml, src/nexus_nancy/__init__.py, CHANGELOG.md, and docs/conf.py
...
def update_pyproject_toml(new_version):
    ...
    # Update version
    content = re.sub(r'version = "\d+\.\d+\.\d+"', rf'version = "{new_version}"', content)
...
def update_conf_py(new_version):
    ...
    content = re.sub(
        r'release = [\'"][0-9]+\.[0-9]+\.[0-9]+[\'"]', f'release = "{new_version}"', content
    )
...
def update_init_py(new_version):
    """Update version in src/nexus_nancy/__init__.py."""
    init_py = Path("src/nexus_nancy/__init__.py")
    if not init_py.exists():
        print("Warning: src/nexus_nancy/__init__.py not found, skipping")
        return
    ...
    content = re.sub(
        r'__version__ = [\'"][0-9]+\.[0-9]+\.[0-9]+[\'"]', f'__version__ = "{new_version}"', content
    )

    init_py.write_text(content)
    print(f"Updated src/nexus_nancy/__init__.py to version {new_version}")
...
def generate_release_notes_from_changelog(version):
    ...
    if "## What's New" not in content and "## What's Included" not in content:
        ...
        release_lines.append("")
        release_lines.append("See [CHANGELOG.md](CHANGELOG.md) for the complete list of changes.")
        release_lines.append("")
        release_lines.append("---")
        release_lines.append("")
        release_lines.append(
...
def main():
    ...
    parser.add_argument(
        "bump_type", choices=["patch", "minor", "major", "release"], help="Type of version bump"
    )
    parser.add_argument(
